    ...
    >
      <div className="w-full max-w-md bg-[#313338] rounded-md shadow-elevation-high p-6 sm:p-8 animate-modal-in">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-app-header mt-4 tracking-tight">Create an account</h1>
          <p className="text-app-header-secondary mt-1 text-sm">Join {brand.app_name} in seconds.</p>
        </div>
        <form onSubmit={submit} className="mt-6 space-y-5">
          <div>
            <label className="block text-eyebrow uppercase font-bold text-app-header-secondary mb-2">
              Username <span className="text-app-red">*</span>
            </label>
            <input
              required
              autoFocus
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[#1e1f22] border border-transparent focus:border-app-500 outline-none rounded px-3 py-2.5 text-app-interactive-active transition-colors"
            />
            <div className="text-tiny text-app-header-secondary mt-1">At least 6 characters.</div>
          </div>
          {(localError || error) && (
          ...
          <div className="text-sm text-app-header-secondary">
            ...
            <Link to="/login" className="text-app-link hover:underline">Log In</Link>
          </div>
        </form>
      </div>
    </div>
